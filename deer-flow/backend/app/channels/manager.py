...
from app.gateway.yqs_commands import classify_yqs_command, format_status_reply, read_yqs_status, run_yqs_direct
...
def _classify_yqs_material_command(text: str) -> str | None:
    return classify_yqs_command(text)
...
class ChannelManager:
    ...
    async def _handle_yqs_material_command(self, msg: InboundMessage, command: str) -> None:
        logger.info("[Manager] handling YQS material command from channel=%s command=%s", msg.channel_name, command)
        try:
            if command == "status":
    ...
    async def _handle_chat(
        ...
        *,
        ...
        if msg.files:
            ...
            if msg.metadata.get("yqs_inbox_handled"):
                logger.info("[Manager] inbound file handled by channel inbox, skipping agent run")
                return
        if extra_context:
