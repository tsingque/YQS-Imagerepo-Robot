...
class FeishuChannel(Channel):
    ...
    async def receive_file(self, msg: InboundMessage, thread_id: str, *, user_id: str | None = None) -> InboundMessage:
        ...
        msg.text = text
        msg.metadata["yqs_inbox"] = inbox_stats
        if msg.files:
            msg.metadata["yqs_inbox_handled"] = True
        if inbox_stats.get("saved"):
            await self._reply_yqs_inbox_receipt(msg.thread_ts, inbox_stats)
        return msg
    ...
    async def _receive_single_file(
        ...
        *,
        ...
        try:
            ...
            if inbox_stats is not None:
                inbox_stats["failed"].append(filename)
            return f"Failed to obtain the [{type}]"

        mirror_result = await self._mirror_to_yqs_inbox(
            filename=filename,
            file_key=file_key,
            type=type,
            content=content,
        ...
        if inbox_stats is not None and mirror_result["status"] in inbox_stats:
            inbox_stats[mirror_result["status"]].append(mirror_result)
    ...
    async def _mirror_to_yqs_inbox(
        self,
        *,
        filename: str,
        file_key: str,
        ...
        inbox_dir = os.getenv("YQS_FEISHU_INBOX_DIR", "").strip()
        if not inbox_dir:
            return {"status": "failed", "filename": filename, "reason": "YQS_FEISHU_INBOX_DIR is not configured"}

        suffix = Path(filename).suffix.lower()
        if type != "image" and suffix not in YQS_IMAGE_EXTENSIONS:
            return {"status": "rejected", "filename": filename, "reason": "unsupported file type"}

        safe_name = re.sub(r"[./\\]", "_", Path(filename).stem).strip("_") or "feishu_image"
        safe_suffix = suffix if suffix in YQS_IMAGE_EXTENSIONS else ".png"
        target_dir = Path(inbox_dir)
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        target = target_dir / f"{timestamp}_{file_key[-8:]}_{safe_name}{safe_suffix}"
        ...
        def write_copy() -> None:
            ...
            with self._thread_lock:
                ...
                self._append_yqs_source_record(target_dir, candidate, filename, file_key, inbound)

        try:
            await asyncio.to_thread(write_copy)
            logger.info("[Feishu] mirrored image to YQS inbox: %s", written_path or target)
            return {"status": "saved", "filename": filename, "path": str(written_path or target)}
        except Exception:
            logger.exception("[Feishu] failed to mirror image to YQS inbox: %s", target)
            return {"status": "failed", "filename": filename, "reason": "write failed"}

    @staticmethod
    def _append_yqs_source_record(
        target_dir: Path,
        saved_path: Path,
        ...
        metadata = inbound.metadata if inbound else {}
        record = {
            "saved_at": time.strftime("%Y-%m-%d %H:%M:%S"),
            "saved_path": str(saved_path),
            "saved_name": saved_path.name,
            "original_filename": original_filename,
            "file_key_tail": file_key[-8:],
            "chat_id": inbound.chat_id if inbound else "",
            "sender_id": inbound.user_id if inbound else "",
            "message_id": metadata.get("message_id", ""),
            "root_id": metadata.get("root_id", ""),
            "thread_id": metadata.get("thread_id", ""),
        }
        with (target_dir / "_feishu_sources.jsonl").open("a", encoding="utf-8") as fh:
            fh.write(json.dumps(record, ensure_ascii=False) + "\n")

    async def _reply_yqs_inbox_receipt(self, message_id: str, stats: dict[str, Any]) -> None:
        saved = stats.get("saved", [])
        pending_count = self._count_yqs_pending_images()
        text = (
            f"**素材已入库**\n\n"
            f"已收到 {len(saved)} 张图片，已保存到 images_raw。\n\n"
            f"当前已有 {pending_count} 张图片在待处理区。"
        )
        await self._reply_card(message_id, text)

    @staticmethod
    def _count_yqs_pending_images() -> int:
        inbox_dir = os.getenv("YQS_FEISHU_INBOX_DIR", "").strip()
        if not inbox_dir:
            return 0
        try:
            return sum(1 for path in Path(inbox_dir).iterdir() if path.is_file() and path.suffix.lower() in YQS_IMAGE_EXTENSIONS)
        except Exception:
            logger.exception("[Feishu] failed to count YQS pending images")
            return 0
