...
- 同事在飞书多维表格上传图片，并填写 `名字 / 描述 / 文件 / 来源 / 是否可商用`。
...
其中 `来源` 和 `是否可商用` 可以在飞书里设置为选项字段。系统会读取选项的显示值。
...
| `文件` | 图片附件 |
| `来源` | 拍摄 / 下载 / 截图 / AI生成 |
| `是否可商用` | `有` 进入处理流程；`无` 同步后跳过 |

多维表格和表单只保留这 5 个输入字段。处理状态、AI 识别结果、本地路径等信息只保存在本地运行状态和素材清单里，不写回飞书表格。

`是否可商用` 是入库开关：`有` 表示已确认有商用权限，可以下载、压缩、识图并进入素材库；`无` 表示不处理、不进入素材库。AI 只读取这个字段，不判断或改变商用权限。
